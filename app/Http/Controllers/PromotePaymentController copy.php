...
use App\Mail\PetPromotionSuccessMail; // Assuming you have this Mailable
use App\Mail\FailedPaymentEmail; // Re-use or create specific for promotion
...
class PromotePaymentController extends Controller
{
    ...
    public function successStripe(Request $request)
    {
        ...
        try {
            ...
            if ($response->payment_status == 'paid') {
                ...
                return to_route('f.dashboard')->with('success', 'Pet promotion successful!');
            } else {
                // Payment not successful according to Stripe
                Mail::to($user->email)->send(new FailedPaymentEmail($pet)); // Or a specific promotion failure email
                return redirect()->route('f.promote.stripe.cancel')->with('error', 'Stripe payment was not successful.');
            }
        } catch (\Exception $e) {
            Mail::to($user->email)->send(new FailedPaymentEmail($pet ?? null)); // Handle case where pet might not be found
    ...
    {
        session()->forget(['promotion_pet_id', 'promotion_amount', 'promotion_name']);
        return redirect()->route('f.dashboard')->with('error', 'Stripe payment cancelled for pet promotion.');
    }
    ...
    {
        session()->forget(['promotion_pet_id', 'promotion_amount', 'promotion_name']);
        return redirect()->route('f.dashboard')->with('error', 'PayPal payment cancelled for pet promotion.');
    }
}
